...
var CONFIG_FILE_PATH = IS_VERCEL ? path.resolve("/tmp", "turso-config.json") : path.resolve(process.cwd(), "turso-config.json");
...
apiRouter.get("/fonts/vietnamese", (_req, res) => {
  try {
    ...
    const regPath = path2.join(process.cwd(), "public", "fonts", "Roboto-Regular.ttf");
    const boldPath = path2.join(process.cwd(), "public", "fonts", "Roboto-Bold.ttf");
    if (fs2.existsSync(regPath) && fs2.existsSync(boldPath)) {
      const regBuf = fs2.readFileSync(regPath);
      const boldBuf = fs2.readFileSync(boldPath);
      cachedFontPayload = {
...
export {
  entry_serverless_default as default
};
